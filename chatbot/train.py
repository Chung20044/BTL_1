...
# Khởi tạo dictionary intents rỗng
intents = {"intents": []}
...
# Lưu danh sách các nhãn tương ứng như greeting, goodbye, ...
tags = []
...
# Xây dựng dữ liệu, mỗi mẫu dữ liệu trong xy chuyển về vector
...
# Số lượng lần duyệt qua toàn bộ tập tin
num_epochs = 50
...
# Kích thước lớp ẩn trong neural network
hidden_size = 32
...
for epoch in range(num_epochs):
    ...
    for (words, labels) in train_loader:
        ...
        # Chuyển tiếp đến Neural Network để dự đoán output
        outputs = model(words)
        ...
        optimizer.zero_grad()  # Làm sạch gradient của tất cả các tham số trong mô hình trước khi tính toán gradient mới.
        loss.backward()  # Tính gradient của hàm mất mát theo tất cả các tham số
        optimizer.step()  # Cập nhật trọng số của mô hình sử dụng giải thuật tối ưu hóa
    
    # Theo dõi và in ra màn hình thông tin về mất mát sau mỗi epoch
    if (epoch+1) % 1 == 0:
...
# Tạo ra một dictionary chứa thông tin cần thiết để lưu trữ mô hình
data = {
...
# In ra thông báo rằng quá trình huấn luyện đã hoàn tất 
